...
function dequytable($data,$parent=0,$lv=''){
    foreach ($data as $val){
        if ($val->parent == $parent){
            echo '<tr>
                <td>'.$lv.$val->name.'</td>
                <td><a  href="'.route('admin.astronomicalentity.delete',$val->id).'" onclick="return deleteConfirm()">Delete</a></td>
            <td><a href="'.route('admin.astronomicalentity.edit',$val->id).'">Edit</a></td>
                </tr>';
